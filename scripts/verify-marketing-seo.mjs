...
const { websiteSolutions } = await jiti.import('../app/constants/websiteSolutions.ts')
...
const routes = ['/', '/modelos-site', '/criacao-de-sites', ...websiteSolutions.map(s => s.path), ...serviceLandings.map(s => `/criacao-de-sites/${s.slug}`)]
for (const route of routes) {
  ...
  if (route === '/') {
    assert.ok(schemas.some(schema => schema['@graph']?.some(item => item['@type'] === 'WebSite' && item.name === 'Avyro')))
    assert.ok(html.includes('todo o Brasil'))
  } else {
    const crumbs = schemas.find(schema => schema['@type'] === 'BreadcrumbList')
    assert.equal(crumbs?.itemListElement.at(-1).item, `https://avyro.com.br${route}`)
  }
  for (const solution of websiteSolutions) assert.ok(html.includes(`href="${solution.path}"`), `Discoverable service: ${solution.path}`)
}
