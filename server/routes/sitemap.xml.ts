...
import { websiteSolutions } from '../../app/constants/websiteSolutions'
export default defineEventHandler(event => {
  const origin = String(useRuntimeConfig(event).public.siteUrl).replace(/\/$/, '')
  const paths = ['/', '/modelos-site', '/criacao-de-sites', ...websiteSolutions.map(item => item.path), ...serviceLandings.map(item => `/criacao-de-sites/${item.slug}`)]
