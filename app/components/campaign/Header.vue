...
<template>
  <header class="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur-lg">
    <div class="mx-auto flex h-20 max-w-[1240px] items-center justify-between gap-6 px-5 md:px-8">
      ...
      <a :href="campaignWhatsapp()" target="_blank" rel="noopener noreferrer" class="hidden items-center gap-3 rounded-lg bg-[#255bef] px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#1949d1] md:inline-flex">Vamos conversar <span aria-hidden="true">↗</span></a>
      <button class="rounded-lg border border-slate-200 px-4 py-2 text-sm md:hidden" :aria-expanded="isOpen" aria-controls="campaign-menu" @click="isOpen = !isOpen">{{ isOpen ? 'Fechar' : 'Menu' }}</button>
    </div>
    <nav v-if="isOpen" id="campaign-menu" aria-label="Navegação no celular" class="space-y-1 border-t border-slate-100 px-5 py-4 md:hidden" @keydown.esc="isOpen = false">
